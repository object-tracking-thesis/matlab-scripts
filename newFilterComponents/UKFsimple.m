...
classdef UKFsimple < handle
   properties
    ...
    methods
        ...
        function this = UKFsimple
        end
        function init(this, Q,R, st0, cov0)
            % Constructor, 
           ...
           % this.nMGPS  = nMGPS;  % How many h-functions do we get
            this.nObsSt = 2; % How many states will we observe through h-functions 
            
            this.nSt = 5;       % Nr of states in statevector             
            this.nSP = 1 + 2*5; % Nr of sigma points to use in filter 
        ...
        function [] = updateMoments(this, hCell, assignedZ)             
            ...
            % Calculate the modified yDiff (DEBUG)
            
            yDiffmod = zeros(this.nObsSt*this.nMGPS, this.nSP);
            ...
            for j = 1:this.nSP
               yD = [];
               for h = 1:this.nMGPS
                  yD = [yD; hCell{h}(SP(:,j))-hCell{1}(SP(:,1))]; 
               end
               yDiffmod(:,j) = yD; % Each column of yDiff should contain h(SP) - ypred for each SP
            end
            ...
            %% Calculate Smod (DEBUG)
            
            Smod = zeros(this.nMGPS*this.nObsSt);            
            for j = 1:this.nSP
                Smod = Smod + yDiffmod(:,j)*yDiffmod(:,j)'*W(j);
            end            
            Smod = Smod + this.R*eye(this.nMGPS*this.nObsSt);
