...
classdef PHDinstance2 < handle
    ...
    methods(Access = public)
        ...
        function this = PHDinstance2(weights, means)
            % Instantiate the PHD filter by generating the birth RFS
            temp(1,length(weights)) = RectTarget;
        ...
        function update(this, Z)            
            ...
            predStorage(1,length(this.componentStorage)) = RectTarget;            
            for j = 1:length(this.componentStorage)
            ...
            updatedStorage(1,a*b) = RectTarget;            
            ...
            for m = 1:size(Z,2) % Measurement index 
               ...
               for j = 1:length(this.componentStorage) % component in prediction                   
                   rectTarget = this.componentStorage(j).copy();
                   totalWL = totalWL + rectTarget.weight*rectTarget.calcLikelihood(Z{m});
               end
               totalWL = this.K + 0.99*totalWL;
               
               for j = 1:length(this.componentStorage) % component in prediction
                   rectTarget = this.componentStorage(j).copy(); % copy over the predicted component 
                   rectTarget.weight = 0.99 * rectTarget.weight * rectTarget.calcLikelihood(Z{m})/totalWL; % Calculate the new weight 
                   rectTarget.update(); % KF update the gaussian component
                   updatedStorage(1,idx) = rectTarget; % Store the component
                   idx = idx + 1;
               end
            ...
            this.componentStorage(([this.componentStorage.weight] > 0.1)); % For some reason this line fucks stuff up
        ...
        function bRFS = getBirthRFS(this)
            ...
            % each timestep
            bRFS(1,length(this.birthRFS)) = RectTarget;
            for j = 1:length(this.birthRFS)
                bRFS(j) = this.birthRFS(j).copy();
            end
        end
        
        function rectTarget = getBestRect(this, threshold)
            idx = find([this.componentStorage.weight] > threshold);
            
            rectTarget = this.componentStorage(idx);
            
            for k = 1:length(rectTarget)
                rectTarget(k) = rectTarget(k).copy();
            end
        end
